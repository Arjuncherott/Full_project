...
# Â  Â  Â ii. Copy it in the same directory as your terraform code
resource "aws_instance" "my_ec2_instance1" {
  ami                    = "ami-0cf10cdf9fcd62d37"
  instance_type          = "t2.medium"
  vpc_security_group_ids = [aws_security_group.my_security_group1.id]
  key_name               = "cost" # paste your key-name here, do not use extension '.pem'
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      private_key = file("./cost") # replace with your key-name 
      user        = "ec2-user"
      host        = self.public_ip
    }
